...
public class PageFetchResult
{
	...
	// LoggerFactory.getLogger(PageFetchResult.class);
	...
	/*
	 * public boolean fetchContent(Page page) { try { page.load(entity);
	 * page.setFetchResponseHeaders(responseHeaders); return true; } catch
	 * (Exception e) { logger.info("Exception while fetching content for: {} [{}]",
	 * page.getWebURL().getURL(), e.getMessage()); } return false; }
	 */
	...
	public void discardContentIfNotConsumed()
	{
		...
		{
			// We can EOFException (extends IOException) exception. It can happen on
			// compressed streams which are not
			// repeatable
